#CS112  Term Project TP2
#Mingnan Xu + Section G + mingnanx
#Animation FrameWork from CMU 15-112 Lecture Nots
...
#Texas Poker instruction
#Click "Start" to Begin, Press "Next" for next round
#Click "Ready" to let AI make decision
#Click "Rematch" when game over
#Press "s" for seeing the simulation probability result to get each hand.
#Press "w" for seeing the largest hand now for AI
#press "q" to flip over the AI card (just for debugging)
...
###Animation FrameWork from CMU 15-112 lecture notes
#### Poker image from lecture notes ####
def init(data):
    #initialize
...
#method to load all the playingcardImage from lecture notes
...
def TXRedrawAll(canvas,data):
    ...
    if data.TXgameover:return
    if data.addChip and not data.check:
        if valueMatch(data):
            canvas.create_text(400,540,text = "Please Add Chip or Check"
                               ,fill="red",
                           font = "bold 25")
        else:
            if getAIChipsSum(data)>getPlayerChipsSum(data):
                canvas.create_text(400,540,text = "Match Raise or Fold",fill="red",
                               font = "bold 25")
            else:
                canvas.create_text(400,540,text = "Re - Check",
                                   fill = "red", font = "bold 25")
    elif len(data.TXPlayer.cards)>=2:
        canvas.create_text(400,540,text = "Please Click on Next",fill = "red",
                           font = "bold 25")
    else:
        canvas.create_text(400,540,text = "Click Start to Begin",fill="red",
                           font = "bold 25")
...
def drawTXButton(canvas,data):
    ...
    canvas.create_text(125,325,text = "CHECK")

    canvas.create_rectangle(100,450,180,500,fill="yellow")
    canvas.create_text(140,475,text="REMATCH")

    canvas.create_rectangle(100,500,180,530,fill="purple")
    canvas.create_text(140,515,text = "TIPS",fill="pink")
...
####################################
# Animation FrameWork from CMU 15-112 class notes
####################################
